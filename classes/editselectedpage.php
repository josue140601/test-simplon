...
class EditSelectedPage extends EditPage
{
	...
	protected function prepareButtons()
	{
		...
		{
			$label = str_replace( "%n%", $this->nSelected, "Mise à jour %n% dossiers" );
	...
	{
		$message = str_replace( array("%succeed%", "%total%"), array( "<strong>".$this->nUpdated."</strong>", "<strong>".$this->nSelected."</strong>" ), "%succeed% de %total% enregistrements mis à jour avec succès.");
		$this->setMessage( $message );
		
		if( $this->nUpdated != $this->nSelected ) {
			$message = str_replace( "%failed%", "<strong>".($this->nSelected - $this->nUpdated)."</strong>" , "%failed% dossiers ont échoué.");
			$this->setMessage( $message );
		}
	}
