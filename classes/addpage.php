...
class AddPage extends RunnerPage
{
	...
	protected function recheckUserPermissions()
	{
		...
		if( isLoggedAsGuest() || !isLogged() )
		{
			$this->setMessage( "Votre session a expiré."
				. "<a href='#' id='loginButtonContinue" . $this->id . "'>" . "Connexion " . "</a>"
	...
	protected function setSuccessfulUpdateMessage()
	{
		...
		if( $this->mode == ADD_INLINE )
			$infoMessage = ""."Enregistrement ajouté"."";
		else
			$infoMessage = "<strong><<< "."Enregistrement ajouté"." >>></strong>";
		...
		else
		{
			...
			if( $this->editAvailable() )
				$infoMessage.= "&nbsp;<a href='".GetTableLink( $this->pSet->getShortTableName(), "edit", $keylink )."'>"."Editer"."</a>&nbsp;";

			if( $this->viewAvailable() )
				$infoMessage.= "&nbsp;<a href='".GetTableLink( $this->pSet->getShortTableName(), "view", $keylink )."'>"."Afficher"."</a>&nbsp;";
		}
	...
	public function setDatabaseError( $message )
	{
		if( $this->mode != ADD_INLINE )
		{
			$this->message = "<strong>&lt;&lt;&lt; "."L'enregistrement n'a pas été ajouté"."</strong> &gt;&gt;&gt;<br><br>".$message;
		}
		else
		{
			$this->message = "L'enregistrement n'a pas été ajouté".". ".$message;
		}
