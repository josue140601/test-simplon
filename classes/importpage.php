...
class ImportPage extends RunnerPage
{
	...
	protected function prepareAndSentPreviewData()
	{
		...
		if( $returnJSON != false )
			...
			echo "Le fichier que vous essayez d'importer n'a pas pu  être analysé";
	...
	protected function getBasicReportText( $totalRecords, $addedRecords, $updatedRecords, $isNotLogFile = true, $lineBreak = "<br>", $errorMessages = array(), $unprocessedData = array() )
	{
		...
		{
			$reportText .= "Importer dans"." ".$this->strOriginalTableName.$lineBreak.
				str_format_datetime( db2time( now() ) ) .$lineBreak.$lineBreak;
		}
		
		$reportText .= mysprintf("%s de %s des fichiers traitées avec succès.", array($boldBegin.$importedReords.$boldEnd, $boldBegin.$totalRecords.$boldEnd)) .$lineBreak.
			mysprintf("%s de enregistrements ajoutés.", array($boldBegin.$addedRecords.$boldEnd)) .$lineBreak.
			mysprintf("%s de enregistrements mis à jour.", array($boldBegin.$updatedRecords.$boldEnd)) .$lineBreak;
			
		if( $notImportedRecords )
			$reportText.= mysprintf("%s de enregistrements avec erreurs.", array($boldBegin.$notImportedRecords.$boldEnd));
