...
*/
	...
	function checkCaptcha()
	{
		...
		if ( $captchaSettings["type"] == FLASH_CAPTCHA && @strtolower($this->captchaValue) != strtolower(@$_SESSION["captcha_" . $this->getCaptchaId()]) )
		{
			...
			$this->message = "Code de sécurité non valable";
		}
	...
	function getCaptchaHtml($_captchaFieldName)
	{
		...
		$typeCodeMessage = "Tapez le code que vous voyez ci-dessus";
		$path = GetCaptchaPath();
...
*/		
	...
	function noRecordsMessage()
	{
		...
		if( $this->pSetSearch->noRecordsOnFirstPage() && !$isSearchRun )
			return "Rien à voir. Rechercher.";

		if( !$this->rowsFound && !$isSearchRun )
			return "Aucune donnée.";

		if( $isSearchRun && !$this->rowsFound )
			return "Pas d'enregistrements trouvés";
	}
	...
	function buildPagination()
	{
		...
		else
		{
			...
			if($this->maxPages > 1)
			{
				...
				if($counterstart != 1)
				{
					$pagination.= $this->getPaginationLink(1,"Premier") . $advSeparator;
					$pagination.= $this->getPaginationLink($counterstart - 1,"Précédent").$separator;
				}
				$pageLinks = "";
				...
				if($counterend != $this->maxPages)
				{
					$pagination.= $separator . $this->getPaginationLink($counterend + 1,"Suivant") . $advSeparator;
					$pagination.= $this->getPaginationLink($this->maxPages,"Dernier");
				}
				if( $this->isBootstrap() )
	...
	function prepareRecordsIndicator($firstDisplayed, $lastDisplayed, $totalDisplayed)
	{
		...
		{
			$template = "Résultats %first%-%last% de %total%";
	...
	*/
	...
	function getProceedLink()
	{
		...
		return '<span class="rnr-dbebrick">'
			...
			.  "Continuez vers" . ' '. GetTableCaption( GoodFieldName( $this->tName ) )
			. '</a>'
			. "&nbsp;&nbsp;</span>";
	}
...
//		$this->AddJSFile('libs/js/migrationTool.js');
	...
	{
		if( $page == "add" )
			return GetTableCaption($table).", "."Ajouter";
		if( $page == "edit" )
			return GetTableCaption($table).", "."Editer"." [". RunnerPage::getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "view" )
			return GetTableCaption($table)." [". RunnerPage::getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "export" )
			return "Exporter";
		if( $page == "import" )
			return GetTableCaption($table).", "."Importer";
		if( $page == "search" )
			return GetTableCaption($table)." - "."Recherche avancée";
		if( $page == "print" )
