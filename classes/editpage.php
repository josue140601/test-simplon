...
/*
	public function getKeysCondition( $useOldKeys )
	{
		if( $useOldKeys )
			return DataCondition::FieldsEqual( $this->pSet->getTableKeys(), $this->oldKeys );
		else
...
*/
	...
	public static function processEditPageSecurity( $table )
	{
		...
		if( $pageMode != EDIT_SIMPLE )
		{
			...
			if( !isLogged() || isLoggedAsGuest() )
				$messageLink = " <a href='#' id='loginButtonContinue'>". "Connexion " . "</a>";
	...
	public static function handleBrokenRequest()
	{
		...
		if( !postvalue("editid1") )
		{
			$returnJSON = array();
			$returnJSON['success'] = false;
			$returnJSON['message'] = "Une erreur s'est produite";
		...
		else
		{
			if( postvalue('fly') )
			{
				...
				$_SESSION["message_edit"] = "<< "."Une erreur s'est produite"." >>";
		}
	}
	...
	protected function setSuccessfulEditMessage()
	{
		...
		$this->setMessage( "<strong>&lt;&lt;&lt; "."Enregistrement modifié". " &gt;&gt;&gt;</strong>" );
	}
	...
	protected function recheckUserPermissions()
	{
		...
		{
			$this->setMessage( "Votre session a expiré." .
	...
	protected function SecurityRedirect()
	{
		if($this->mode == EDIT_INLINE)
		{
			echo printJSON(array("success" => false, "message" => "L'enregistrement ne peut être modifié"));
			exit();
		}
	...
	public function setDatabaseError( $message )
	{
		if( $this->mode != EDIT_INLINE )
		{
			$this->message = "<strong>&lt;&lt;&lt; "."L'enregistrement n'a pas été modifié"."</strong> &gt;&gt;&gt;<br><br>".$message;
		}
		else
			$this->message = "L'enregistrement n'a pas été modifié".". ".$message;
