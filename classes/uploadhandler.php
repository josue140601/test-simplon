...
class UploadHandler
{
    ...
    public function validate($uploadedFile, &$file, $error, $file_size, $index, $uploadDir) {
        ...
        if (!$file["name"]) {
            $file["error"] = "Aucun nom de fichier n'était indiqué";
            return false;
        }
        if (!preg_match($this->options['accept_file_types'], $file["name"])) {
            $file["error"] = "Type de fichier inacceptable";
            return false;
        }
        if ($this->options['max_file_size'] && (
            ...
            $file["error"] = mysprintf("Largeur du fichier excède la limite de %s kilo-octets", array($this->options['max_file_size']));
            return false;
        }
        if ($this->options['min_file_size'] &&
            $file_size < $this->options['min_file_size'] * 1024) {
            $file["error"] = mysprintf("Largeur du fichier ne peut pas être inférieur de %s kilo-octets", array($this->options['min_file_size']));
            return false;
        }
   		if (is_int($this->options['max_totalFile_size']) && (
                $this->getUploadFilesSize() + $file["size"] > $this->options['max_totalFile_size'] * 1024)
            ) {
            $file["error"] = mysprintf("Largeur total des fichiers excède la limite de %s kilo-octets", array($this->options['max_totalFile_size']));
            return false;
        }
        if (is_int($this->options['max_number_of_files']) && (
            	...
            	if($this->options['max_number_of_files'] > 1)
            		$file["error"] = mysprintf("Vous ne pouvez télécharger que %s fichiers", array($this->options['max_number_of_files']));
            	else 
            		$file["error"] = "Vous ne pouvez télécharger qu'un seul fichier";
            return false;
        }
    ...
    public function handle_file_upload($uploadedFile, $index) {
		...
		if($this->pSet->isMakeDirectoryNeeded($this->field))
		{
			...
			{
				$file["error"] = "Le répertoire de téléchargement n'existe pas";
				return $file;
			}
		}
