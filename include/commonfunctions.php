...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("test-e");
...
function verifyRecaptchaResponse( $response ) {
	...
	$errors = array();
	$errors["missing-input-response"] = "Code de sécurité non valable";
	$errors["invalid-input-response"] = "Code de sécurité non valable";
